  ...
  it('renders the structured debug sections for a traced run', () => {
    const html = formatDebugEmail({
      ...
      ai: {
        ...
        rawFallbackResponse: '{"editorial":"Good night."}',
        rawPrimaryResponse: '{"editorial":"Good night.","composition":["Low northern horizon"]}',
        rawPrimaryPayload: '{"candidates":[{"content":{"parts":[{"text":"{\\"editorial\\":\\"Good night.\\"}","thoughtSignature":"sig"}]}}],"usageMetadata":{"thoughtsTokenCount":261}}',
  ...
  it('shows the explicit primary rejection reason when a secondary model is selected', () => {
    const html = formatDebugEmail({
      ...
      ai: {
        ...
        rawFallbackResponse: '{"editorial":"Conditions hold through the astro window.","composition":["Face north with a low tree line"]}',
        rawPrimaryResponse: '{"editorial":"The moon sets before the midnight astro window begins at 00:',
        rawPrimaryPayload: '{"body":{"data":{"candidates":[{"content":{"parts":[{"text":"truncated"}]}}]}}}',
