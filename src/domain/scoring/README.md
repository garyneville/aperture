...
- `summarize-day.ts` falls back to 20 for `total_column_integrated_water_vapour` when absent. The UKMO model does not support this field, so it is not requested in the Weather node URL.
- `summarize-day.ts` reads `boundary_layer_height` and `soil_temperature_0cm` from the weather object. These fields are provided by the `HTTP: ECMWF Supplement` node (ECMWF IFS 0.25°) and merged into the weather data by `build-score-input`. When the ECMWF supplement is unavailable, `boundary_layer_height` degrades to `null` (mist scoring via dew-point spread) and `soil_temperature_0cm` degrades to `null` (`hasFrost` returns `null`).
